...
namespace AutoTraderSDK.Kernel
{
    ...
    public class TXMLConnectorWrapper: IDisposable //, AutoTraderSDK.Kernel.ITXMLConnectorWrapper
    {
        #region CONSTRUCTOR
        ...
        public static TXMLConnectorWrapper GetInstance()
        {
            TXMLConnectorWrapper res = null;

            if (_txmlconnector1 == null)
            {
                _txmlconnector1 = new TXMLConnectorWrapper();
                res = _txmlconnector1;
            }
            else
            {
        ...
        public void Login(string username, string password, string server = "tr1.finam.ru", int port = 3900)
        {
            ...
            else
            {
                while (true)
                {
                    ...
                    try
                    {
                        serverStatus = (server_status)_deserialize(_serverStatus, typeof(server_status));
                    }
                    catch { }
        ...
        [DllImport("txmlconnector1.dll", CallingConvention = CallingConvention.Winapi)]
        static extern IntPtr Initialize(IntPtr pPath, Int32 logLevel);
